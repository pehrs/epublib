...
public class XHTMLTocParser {
    ...
    private static final Logger log = LoggerFactory.getLogger(XHTMLTocParser.class);
	...
	if(MediatypeService.XHTML.equals(mediaType)) {
		...
		Element nav = DOMUtil.getFirstElementByTagNameNS(root, 
								 ...
								 "nav");
		if(nav==null) {
		    // A lot of them do not use the qualified name <epub:nav> but only '<nav>'
    ...
    /**
     * Parse the top <OL> element in the XHTML
     * @param topOl
     */
    ...
    private static void readTOCReferences(Book book, NodeList liChilds, List<TOCReference> tocRefs) {
	for(int lc=0;lc<liChilds.getLength();lc++) {
		...
		if (resource == null) {
		    log.error("Resource with href " + href + " in XHTML TOC document not found");
		}

		log.debug("new TOCReference: "+title+", resource="+resource);
		TOCReference tocReference = new TOCReference(title, resource, fragmentId);
